...
It covers the full campaign workflow — importing and filtering recipients, composing rich-text emails, reusable templates, attachments, paced/scheduled sending, and follow-up sequences. Email sending and Gmail-related operations use your own Gmail account via OAuth.
...
flowchart TB
    ...
    subgraph cluster["Self-managed Kubernetes Cluster · kubeadm + containerd + Calico"]
        ...
        metrics["Metrics Server"] -. metrics .-> hpa["HPA · cache-runtime<br/>min 1 / max 20 replicas<br/>CPU target 50%<br/>(illustrative scaled-out state shown)"]
        hpa -. scales .-> e1
...
Metrics Server feeds the resource metrics the HPA reads. The HPA is configured for 1–20 replicas, and scale-out and scale-in have been verified under load. In the captured test, the backend scaled out to 9 replicas before scaling back down after the load stopped. Full test method and results: **[Autoscaling documentation](docs/autoscaling.md)**.
...
- **[Autoscaling](docs/autoscaling.md)** — Metrics Server, HPA configuration, and verified scale-out/scale-in
- **[Troubleshooting](docs/troubleshooting.md)** — real infrastructure failures and the fixes used during the build

## Screenshots

Since the Kubernetes cluster and its AWS resources are temporary, screenshots are preserved under `docs/evidence/` and referenced from the relevant documentation pages so the results outlive the cluster itself.

- **Cluster** — nodes, workloads, and pod placement across both EC2 instances
- **Kubernetes networking** — Services, NodePort, ClusterIP, and ALB Ingress
- **Load balancing & TLS** — ALB target group health, ACM certificate status, HTTPS access in the browser
- **Autoscaling** — HPA baseline, scale-out, and scale-in
